...
$(function() {
  ...
  function update(e) {
    ...
    maxHitDmg = maxHitDmg.join("");
      ...
      mharray.splice([i], 0, (combatant.name + "-" + maxHitName));
      mhdigits.splice([i], 0, (maxHitDmg));
      dmgarray.splice([i], 0, parseInt(maxHitDmg));

      var maxhitnolb = (mharray[indexOfMax(dmgarray)]);
      var maxhitdigits = (mhdigits[indexOfMax(dmgarray)]);
    ...
    if (ignoreLimitBreak == true) {
      ...
      header.find('.maxhitdmg').text(maxhitdigits);
    } else {
